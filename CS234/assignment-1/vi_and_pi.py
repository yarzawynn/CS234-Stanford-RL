...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for s in range(nS):
            action = policy[s]
            prob, next_s, reward, terminal = P[s][action][0]
            value_function[s] = reward + gamma * prob * previous_value_function[next_s]
        if np.max(value_function - previous_value_function) < tol:
            break
    ############################
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for s in range(nS):
        ...
        for a in range(nA):
            ...
            action.append(a)
        new_policy[s] = action[np.argmax(action_reward)]
    ...
    while True:
        ...
        for s in range(nS):
            ...
            for a in range(nA):
                prob, next_s, reward, terminal = P[s][a][0]
                action.append(a)
                action_rewards.append(reward + gamma * prob * prev_value_function[next_s])
            value_function[s] = np.max(action_rewards)
            policy[s] = action[np.argmax(action_rewards)]
